...
var PLACE_HOLDER_REGEXP = new RegExp(
  '(\\\\)?"@__(F|R|D|M|S|A|U|I|B|L|E)-' + UID + '-(\\d+)__@"',
  'g',
)
...
export default function serialize(obj, options) {
  ...
  function replacer(key, value) {
    ...
    if (type === 'object') {
      ...
      try {
        if (origValue instanceof Element) {
          return '@__E-' + UID + '-' + (elements.push(origValue) - 1) + '__@'
        }
      } catch (error) {}
    }
  ...
  return str.replace(
    ...
    function (match, backSlash, type, valueIndex) {
      ...
      if (type === 'R') {
        ...
        var flags = String(regexps[valueIndex].flags).replace(
          /[^gimsuydv]/g,
          '',
        )
        return (
      ...
      if (type === 'L') {
        return (
          'new URL(' + serialize(urls[valueIndex].toString(), options) + ')'
        )
      }

      if (type === 'E') {
        try {
          return elements[valueIndex].cloneNode().outerHTML
        } catch (error) {
          return ''
        }
      }
