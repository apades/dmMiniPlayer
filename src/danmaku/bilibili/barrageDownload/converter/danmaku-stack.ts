...
export class DanmakuStack {
  ...
  getTags(
    ...
    */
    const visibleTime =
      (this.duration(danmaku) * width) / (this.resolution.x + width) +
      DanmakuStack.nextDanmakuDelay
    let trackNumber = initTrackNumber
    let overlayDanmaku = null
    // 寻找前面已发送的弹幕中可能重叠的
    const overlayDanmakuInTrack = (it: TrackItem) =>
    ...
    return this.getTags(danmaku, {
      ...
      willOverlay: (it: HorizontalTrackItem, trackNumber, width) => {
        ...
        if (it.width < width) {
          ...
          */
          return (
            (this.duration(danmaku) * this.resolution.x) /
